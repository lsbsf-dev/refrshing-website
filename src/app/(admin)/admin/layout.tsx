...
function AdminLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    ...(hasPermission(profile.role, Permissions.Ministers.Read) ? [{ label: "Ministers", href: "/admin/ministers", icon: Users }] : []),
  ...
  return (
    <div className={`min-h-screen flex flex-col lg:flex-row font-sans transition-colors duration-300 ${isDark ? "dark" : ""} ${isDark ? "bg-[#0B0907] text-[#FCFAF6]" : "bg-[#F7F4EE] text-[#0B0907]"}`}>
      ...
      <aside className={`hidden lg:flex flex-col border-r shrink-0 p-4 transition-all duration-300 z-30 sticky top-0 h-screen overflow-y-auto custom-scrollbar ${isCollapsed ? "w-20" : "w-60"} ${isDark ? "bg-[#12100C] border-white/10" : "bg-white border-black/10 shadow-sm"}`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3.5 py-3 rounded-xl font-sans text-xs font-semibold uppercase tracking-wider transition-all duration-200 active-press ${isActive ? "bg-[#C25627] text-white shadow-md font-bold" : isDark ? "text-white/70 hover:bg-white/5 hover:text-white" : "text-zinc-600 hover:bg-black/5 hover:text-zinc-900"} ${isCollapsed ? "justify-center" : ""}`}
              >
        ...
        <div className="mt-6 pt-4 border-t border-black/10 dark:border-white/10 flex flex-col gap-2">
          <Link
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-sans transition-colors ${isDark ? "bg-white/5 hover:bg-white/10 text-white/80" : "bg-black/5 hover:bg-black/10 text-zinc-700"} ${isCollapsed ? "justify-center" : "justify-between"}`}
          >
      ...
      <div className={`lg:hidden border-b p-4 flex items-center justify-between z-40 sticky top-0 ${isDark ? "bg-[#12100C] border-white/10" : "bg-white border-black/10 shadow-xs"}`}>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold uppercase ${isActive ? "bg-[#C25627] text-white font-bold" : isDark ? "text-white/70 hover:bg-white/5" : "text-zinc-700 hover:bg-black/5"}`}
                >
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        <header className={`h-16 border-b backdrop-blur-md sticky top-0 z-20 px-3 sm:px-6 flex items-center justify-between transition-colors ${isDark ? "bg-[#12100C]/90 border-white/10" : "bg-white/90 border-black/10 shadow-xs"}`}>
          <div className="flex items-center gap-3">
            <button onClick={() => setIsCollapsed(!isCollapsed)} className={`hidden lg:flex p-2 rounded-xl transition-colors cursor-pointer ${isDark ? "text-white/70 hover:text-white hover:bg-white/10" : "text-zinc-600 hover:text-zinc-900 hover:bg-black/5"}`} title={isCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={toggleTheme} className={`p-2 rounded-xl border transition-all cursor-pointer flex items-center gap-2 text-xs font-sans font-bold uppercase ${isDark ? "bg-white/5 border-white/15 text-[#DDB94E] hover:bg-white/10" : "bg-zinc-100 border-zinc-300 text-zinc-800 hover:bg-zinc-200"}`} title="Toggle Light / Dark Mode">
              {isDark ? (
                <>
