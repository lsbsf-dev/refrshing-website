...
import { getEventScopedDocs, setEventScopedDoc, deleteEventScopedDoc, BibleStudy } from "@/lib/firebase/cms";
...
export default function BibleStudiesAdminPage() {
  ...
  const handleEdit = (item: BibleStudy) => {
    setEditingItem(item);
    setTitle(item.title);
    setTheme(item.theme);
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      await setEventScopedDoc(selectedEventId, "bibleStudies", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "bibleStudies", selectedEventId] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      setIsDeletingId(id);
      await deleteEventScopedDoc(selectedEventId, "bibleStudies", id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin", "bibleStudies", selectedEventId] });
  ...
  const columns: Column<BibleStudy>[] = [
  ...
  return (
    <div className="pb-10">
      <AdminDataTable
        title="Bible Studies"
        description="Manage bible study materials for the programme."
        data={bibleStudies}
        columns={columns}
        isLoading={isLoading}
      ...
      {isModalOpen && (
        <Modal
          ...
          title={editingItem ? "Edit Bible Study" : "Add Bible Study"}
        >
